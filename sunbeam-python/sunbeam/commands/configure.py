    ...
    def prompt(
        ...
        if has_network_nodes or is_network_node:
            ...
            if is_network_node:
                ...
                LOG.debug("Network nodes exist in cluster, setting local access for compute node")
        else:
            ...
            self.variables["user"]["plan_to_add_network_nodes"] = plan_to_add_network_nodes

            if plan_to_add_network_nodes:
                # User plans to add network nodes, set local access for compute nodes
                self.variables["user"]["remote_access_location"] = utils.LOCAL_ACCESS
                LOG.debug("User plans to add network nodes, setting local access for compute node")
            else:
