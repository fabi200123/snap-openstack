...
class Snap(object):
    ...
    def _snap(self, command: str, optargs: Optional[Iterable[str]] = None) -> str:
        ...
        """
        optargs = optargs or []
        args = ["sudo", "snap", command, self._name, *optargs]
        try:
    ...
    def ensure(
        ...
        if state not in (SnapState.Present, SnapState.Latest):
            ...
            if self._state not in (SnapState.Present, SnapState.Latest):
                ...
                self._refresh(
                    channel=channel, cohort=cohort, revision=revision, devmode=devmode
                )
...
def add(
    ...
    return _wrap_snap_operations(
        snap_names, state, channel, classic, devmode, cohort, revision
    )
